...
void Polynom::write(fstream& file)
{
	...
	//сначала записываем степень многочлена, а потом его коэффициенты по очереди
...
void Polynom::read(fstream& file)
{
	...
	//проверяем считанную степень
	if (tempDegree < 0)
		throw exception("Bad degree");

	_degree = tempDegree;
	//пересоздаем массив коэффициентов (т.к. считана новая степень)
	delete[] _coefficients;
	_coefficients = new double[_degree + 1];
	//считываем элементы из файла по очереди
	for (int i = 0; i < _degree + 1; i++)
...
{
	//выводим только коэффициенты
	for (int i = 0; i < polynom._degree + 1; i++)
		stream << polynom._coefficients[i] << " ";

	//возвращаем ссылку на поток вывода
	//это нужно для того, чтобы делать множественные выводы
	//к примеру, cout << polynom1 << "Привет!" << 1 << endl;
	return stream;
}
...
{
	//сначала записываем степень многочлена (т.к. программа должна уметь корректно считывать данные из файла обратно)
	stream << polynom._degree << " ";

	//потом выводим коэффициенты
	for (int i = 0; i < polynom._degree + 1; i++)
		stream << polynom._coefficients[i] << " ";
...
istream& operator>>(istream& stream, Polynom& polynom)
{
	...
	int tempDegree;
	//сначала считываем степень многочлена
	stream >> tempDegree;
	//проверяем считанную степень
	if (tempDegree < 0)
		throw exception("Bad degree");
	...
	//потом считываем коэффициенты
	for (int i = 0; i < polynom._degree + 1; i++)
		stream >> polynom._coefficients[i];

	//возвращаем ссылку на поток вывода
	//это нужно для того, чтобы делать множественные вводы
	//к примеру, cin >> polynom1 >> str >> var;
	return stream;
}
